...
def load_database():
    ...
    if not os.path.exists(config.VECTOR_DB_DIR):
        print(f"Erro: Diretório do banco de vetores não encontrado em '{config.VECTOR_DB_DIR}'")
...
def search_chunks(vectordb, query):
    ...
    print(f"Buscando por: '{query}' (Recuperando {config.SEARCH_K_RAW} chunks para análise...)")
    
    try:
        results_with_scores = vectordb.similarity_search_with_score(query, k=config.SEARCH_K_RAW)
        ...
        print(f"\nExibindo os {config.SEARCH_K_FINAL} resultados MAIS RELEVANTES (de {config.SEARCH_K_RAW} analisados):")
        
        for i, (doc, score) in enumerate(top_k_results):
            source = doc.metadata.get('source', 'N/A')
            print("\n" + "-" * 50)
            print(f"Resultado Relevante {i+1} (Fonte: {source}) (Score: {score:.4f})")
            print("-" * 50)
