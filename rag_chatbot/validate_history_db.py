...
4. Usar SQLModel.
5. Nova funcionalidade: Resumo Estatístico dos Feedbacks.
"""
...
def run_feedback_summary():
    ...
    st.info("Estatísticas consolidadas de satisfação do usuário.")

    if st.button("Calcular Estatísticas"):
        with get_session_sync() as session:
            # 1. Total de Interações (Respostas do Bot)
            total_msgs = session.exec(select(func.count(ChatHistory.id))).one()

            if total_msgs == 0:
                st.warning("O histórico de conversas está vazio.")
                return

            # 2. Contagem de Feedbacks (Likes e Dislikes)
            likes = session.exec(
                select(func.count(Feedback.id)).where(Feedback.rating == "like")
            ).one()

            dislikes = session.exec(
                select(func.count(Feedback.id)).where(Feedback.rating == "dislike")
            ).one()

            # 3. Cálculo de "Em Branco"
            ...
            data = [
                {
                    "Métrica": "👍 Likes (Positivo)",
                    "Total": likes,
                    "Porcentagem": f"{pct_likes:.2f}%",
                },
                {
                    "Métrica": "👎 Dislikes (Negativo)",
                    "Total": dislikes,
                    "Porcentagem": f"{pct_dislikes:.2f}%",
                },
                {
                    "Métrica": "⬜ Em Branco (Sem Avaliação)",
                    "Total": blanks,
...
def run_export_csv():
    ...
    st.subheader("Modo 5: Exportar Histórico Completo")
